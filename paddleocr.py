...
SUPPORT_DET_MODEL = ['DB'] # 아마도 DB가 Differentiable Binarization 일 것이다. detection 알고리즘인 것 같다.
VERSION = '2.7.0.3'
SUPPORT_REC_MODEL = ['CRNN', 'SVTR_LCNet']
BASE_DIR = os.path.expanduser("~/.paddleocr/") # 리눅스에서 ~는 root를 의미 (해보니깐 윈도우에서는 안되네)
...
def get_model_config(type, version, model_type, lang):
    """_summary_
    위에 MODEL_URLS 딕셔너리에서 type, version, model_type, lang에 맞는 항목을 찾아서 반환
    조건에 맞는 항목이 없는 경우 예외처리 함
    Args:
        type (_type_): OCR, ...
        version (_type_): PP-OCRv4
        model_type (_type_): det, rec, cls
        lang (_type_): ch, en, korean
    ...
    """
    # print(f"model_config_args={type, version, model_type, lang}")
    if type == 'OCR':
    ...
    if version not in model_urls:
        version = DEFAULT_MODEL_VERSION # 버전이 없다면 기본 버전으로
    if model_type not in model_urls[version]: # 원하는 버전에 대해 원하는 모델 (det, rec, cls 등)이 없으면
        if model_type in model_urls[DEFAULT_MODEL_VERSION]: #  기본 버전에 있다면
            version = DEFAULT_MODEL_VERSION                    # 기본 버전 사용
        else:   # 그래도 없으면 에러
            logger.error('{} models is not support, we only support {}'.format(
                model_type, model_urls[DEFAULT_MODEL_VERSION].keys()))
            sys.exit(-1)

    if lang not in model_urls[version][model_type]:                                 # 마찬가지로 원하는 버전에 원하는 언어가 있는지 예외처리
...
class PaddleOCR(predict_system.TextSystem):
    def __init__(self, **kwargs):
        ...
        # 여기서 params.det_model_dir = None
        
        params.det_model_dir, det_url = confirm_model_dir_url(
            params.det_model_dir,
            os.path.join(BASE_DIR, 'whl', 'det', det_lang), ## default model directory
            det_model_config['url']) ## model download link
        
        """_summary_
        모델이 있는 경로, 기본 경로, 다운 링크 등을 넘기면 잘 정리해서 최종적으로 모델이 저장되는 경로와 다운 url을 반환
        """
        # 여기서 params.det_model_dir = /root/.paddleocr/whl/det/ch/ch_PP-OCRv4_det_infer
        # 여기서 det_rul = https://paddleocr.bj.bcebos.com/PP-OCRv4/chinese/ch_PP-OCRv4_det_infer.tar        
        ...
        ### (here) params.use_onnx = False 
        if not params.use_onnx:
            # 잘은 모르겠지만 onnx가 다른 프레임 워크에서 만든 모델을 호환하여 사용할 수 있게 하는 기능이라는 것을 고려할 때
            # 다른 프레임 워크의 모델을 가져오는 것일까?.... 잘 모르겠음
        ...
        if params.det_algorithm not in SUPPORT_DET_MODEL: # 원하는 알고리즘이 지원 목록에 없으면 꽝!
            # 한 가지 알게 된 점은 config 방식으로 여러 설정을 받아서 동작하는 경우에 예외처리를 다 해주는구나 싶다.
            # 나도 다음에 config를 사용할 때 예외 처리를 빡세게 해야 겠음
        ...
        super().__init__(params) # 위에서 설정된 정보가 이 함수 안에서 사용되어 detector, recognition model 등이 생성 및 세팅 됨
...
def main(image_dir):
    ...
    # image_dir = args.image_dir # 기존 코드 주석 처리
    ################## MH MODIFICATION END

    if is_link(image_dir):
        download_with_progressbar(image_dir, 'tmp.jpg') # 경로로 부터 이미지 다운
        image_file_list = ['tmp.jpg']
    else:
        image_file_list = get_image_file_list(image_dir) # dir 내의 이미지 파일들의 리스트를 반환
    ...
    ################## MH MODIFICATION START
    return [image_file_list, results]
    ################## MH MODIFICATION END
