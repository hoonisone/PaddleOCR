...
def is_link(s): # 링크인지 체크를 생각 보다 단순하게 하네
...
def confirm_model_dir_url(model_dir, default_model_dir, default_url):
    ...
    Args:
        model_dir (_type_): None이거나 모델이 있는 경로
        default_model_dir (_type_): _description_
        default_url (_type_): _description_
    ...
    if model_dir is None or is_link(model_dir):
        if is_link(model_dir):
            url = model_dir
        file_name = url.split('/')[-1][:-4] # 파일 이름을 url에서 잘라서 사용하네
